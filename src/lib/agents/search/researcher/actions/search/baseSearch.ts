  ...
  if (input.mode === 'speed' || input.mode === 'balanced') {
    ...
    const pickerResponse = await input.llm.generateObject<typeof pickerSchema>({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `<queries>${input.queries.join(', ')}</queries>\n<search_results>${allSearchResults.map((result, index) => `<result indice=${index}>${JSON.stringify(result)}</result>`).join('\n')}</search_results>`,
        },
      ],
    });
    ...
    const extractedFacts = await Promise.all(
      filteredResults.map(async (result) => {
        try {
          ...
          if (!scrapedData) return { ...result, content: '' };
          ...
          const extractedFactsFromChunks = await Promise.all(
            chunks.map(async (chunk) => {
              try {
                ...
                return extractorOutput.extracted_facts;
              } catch (err) {
          ...
          return {
            ...result,
            content: extractedFactsFromChunks.join('\n'),
          };
        } catch (err) {
          console.log(
