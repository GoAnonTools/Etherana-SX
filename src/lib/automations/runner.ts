...
const getNewestRunnableAutomations = (automations: AutomationRecord[]) => {
...
const executeAutomationWithAgent = async ({
  ...
  const systemInstructions = [
    ...
    'You are running a saved Etherana SX automation. Produce the final reusable output directly.',
  ]
    .filter(Boolean)
    .join('\n\n');
...
const buildPreparedOutputContent = ({
  ...
  return [
    ...
    '## Next Improvement',
    '',
    'Connect more workspace context, sources, or project files to make this automation smarter next time.',
  ].join('\n');
};
...
export const runAutomationById = async (
  ...
  let outputContent = buildPreparedOutputContent({
    automation,
    prompt,
    trigger,
    startedAt,
